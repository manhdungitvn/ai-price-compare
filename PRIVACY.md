...
| Recent searches, saved products, price history | Created by your actions in the side panel | Used for the "Recent" list, the ❤️ Saved tab and price-drop alerts | `chrome.storage.local` on your device |
...
In Settings you can enter your **own** affiliate link templates. When enabled, links that open a store from the side panel or the on-page card carry that affiliate code, and the side panel footer says so. The price you pay is unchanged. The extension ships with no affiliate codes of its own.
...
- **Tìm gần đây, sản phẩm đã lưu, lịch sử giá, ví mã giảm giá** lưu trong `chrome.storage.local` trên máy bạn. Mã giảm giá chỉ được thử ở trang thanh toán **khi bạn bấm "Thử mã"**; extension chỉ nhập mã và bấm "Áp dụng" — không bao giờ tự đặt hàng hay thanh toán.
...
Bạn có thể tự nhập mẫu link affiliate **của bạn** trong Cài đặt. Khi bật, link mở sàn sẽ mang mã đó và chân trang side panel ghi rõ. Giá bạn trả không đổi. Extension không cài sẵn mã affiliate nào.
